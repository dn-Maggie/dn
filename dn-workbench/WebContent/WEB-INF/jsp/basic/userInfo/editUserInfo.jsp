		...
		$("#submit").click(function() {
			...
			if(fileName.length>1){  
				...
				reader.onload = function(e) {
					...
					$('#userInfoFormEdit').ajaxSubmit(options);// 将options传给ajaxForm
				}
				// 读取File对象的数据
		...
		<div class="ui-table ui-widget ui-corner-all ui-border">
			<table class="table">
				<tr>
					...
					<td class="inputTd">
					<input
						id="userId" name="id" type="hidden"
						class="text" value="${userInfo.id}" />
					<input id="edit_userAccount"
				...
				<tr>
					...
					<td class="inputTd"><input id="edit_orgName" name="orgName"
						type="text" class="text" value="${userInfo.orgName}"  readonly="readonly"/> <input
						id="edit_orgId" name="orgId" type="hidden" class="text"
						value="${userInfo.orgId}" />
						<input
						id="orgClass" name="orgClass" type="hidden" class="text"
						value="${userInfo.orgClass}" />
						</td>
			    </tr>
				<tr>
					...
					<td class="inputTd"><select id="edit_states" name="states"
						class="select">
							<option value="1" <c:if test="${userInfo.states eq 1}" >selected</c:if>>启用</option>
							<option value="0" <c:if test="${userInfo.states eq 0}" >selected</c:if>>停用</option>
					</select></td>
			    </tr>
				<tr>
					...
					<td class="inputTd"><select id="roleId" name="roleId"
						class="select">
							<c:forEach var="role" items="${roleList}">
								<option value="${role.roleId}" <c:if test="${userInfo.roleId eq role.roleId}" >selected</c:if>>
...
<%-- 							<option value="1" <c:if test="${userInfo.schooling eq 1}" >selected</c:if>>小学</option> --%>
<%-- 							<option value="2" <c:if test="${userInfo.schooling eq 2}" >selected</c:if>>初中</option> --%>
<%-- 							<option value="3" <c:if test="${userInfo.schooling eq 3}" >selected</c:if>>高中（职高、高</option> --%>
<%-- 							<option value="4" <c:if test="${userInfo.schooling eq 4}" >selected</c:if>>中专大专（高职）</option> --%>
<%-- 							<option value="5" <c:if test="${userInfo.schooling eq 5}" >selected</c:if>>本科</option> --%>
<%-- 							<option value="6" <c:if test="${userInfo.schooling eq 6}" >selected</c:if>>硕士研究生</option> --%>
<%-- 							<option value="7" <c:if test="${userInfo.schooling eq 7}" >selected</c:if>>博士研究生</option> --%>
<!-- 					</select></td> -->
...
<%-- 								<c:if test="${userInfo.salStandardId eq salStandard.id}" >selected</c:if> --%>
<%-- 								>${salStandard.standardName}</option> --%>
<%-- 							</c:forEach> --%>
<!-- 					</select> <input id="salStandardName" name="salStandardName" -->
<%-- 						type="hidden" value="${userInfo.salStandardName}" /></td> --%>
<!-- 			    </tr> -->
<!-- 				<tr> -->
...
<!-- 						</div>		 -->
				...
				<tr>
					<td class="inputTd" colspan="5" style="text-align: center;"><input
						id="submit" type="button" class="ti_bottom" value="保存" />
							<input id="passwordReset" type="button" class="ti_bottom" value="密码重置"/>
					</td>
				</tr>
			</table>
		</div>
