	...
	<div class="ui-table ui-widget ui-corner-all ui-border">
			<table class="table">
				...
				<tr>
					...
					<td class="inputTd">${userInfo.orgName}
						</td>
					<td class="inputLabelTd">性别：</td>
					<td class="inputTd">
					        <c:if test="${userInfo.sex=='男'}" >男</c:if>
							<c:if test="${userInfo.sex=='女'}" >女</c:if></td>
			    </tr>
				<tr>
				...
				<tr>
					...
					<td class="inputTd"><c:if test="${userInfo.states eq 1}" >启用</c:if>
							<c:if test="${userInfo.states eq 0}" >停用</c:if>
					</td>
			    </tr>
				<tr>
...
<!-- 					<td class="inputTd"> -->
<%-- 							<c:if test="${userInfo.schooling eq 1}" >小学</c:if> --%>
<%-- 							 <c:if test="${userInfo.schooling eq 2}" >初中</c:if> --%>
<%-- 							 <c:if test="${userInfo.schooling eq 3}" >高中（职高、高）</c:if> --%>
<%-- 							 <c:if test="${userInfo.schooling eq 4}" >中专大专（高职）</c:if> --%>
<%-- 							 <c:if test="${userInfo.schooling eq 5}" >本科</c:if> --%>
<%-- 							 <c:if test="${userInfo.schooling eq 6}" >硕士研究生</c:if> --%>
<%-- 							 <c:if test="${userInfo.schooling eq 7}" >博士研究生</c:if> --%>
<!-- 					</td> -->
...
<!-- 			    </tr> -->
			    <tr>
					<td colspan="4" class="inputTd">
						<img alt="图片" style="height:400px;width: 100%" src="${userInfo.idCardImgPath}">            
					</td>
				</tr>
<!-- 			     <tr> -->
...
<!-- 			    </tr> -->
				<tr>
					...
					<input class="ti_bottom"  type="button" id="close_button"  value="关闭"/></td>
				</tr>
			</table>
		</div>
</body>
