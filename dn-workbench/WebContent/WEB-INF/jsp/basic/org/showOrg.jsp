	...
	<div class="ui-table ui-widget ui-corner-all ui-border">
		...
		<tr>
		    <td class="inputLabelTd">上级机构：</td>
				...
				<td class="inputTd">
					<c:if test="${org.orgType eq 1}" >公司</c:if>
					<c:if test="${org.orgType eq 2}" >部门</c:if>
				</td>
				
			</tr>
			<tr>
				<td class="inputLabelTd">机构编码：</td>
				<td class="inputTd">${org.orgNo}
				</td>
				<td class="inputLabelTd">机构名称：</td>
				<td class="inputTd">${org.orgName}
				</td>
			</tr>
			<tr>
				<td class="inputLabelTd">机构全称：</td>
				<td class="inputTd">${org.orgFullName}
				</td>
				<td class="inputLabelTd">拼音码：</td>
				<td class="inputTd">${org.pinyin}
				</td>
			</tr>
			<tr>
				<td class="inputLabelTd">单位描述：</td>
				<td class="inputTd">${org.orgDesc}
				</td>
				<td class="inputLabelTd">单位状态：</td>
				<td class="inputTd">
				    <c:if test="${org.orgState eq 1}" >启用</c:if>
				    <c:if test="${org.orgState eq 0}" >停用</c:if>
				</td>
			</tr>
			<tr>
				<td class="inputLabelTd">机构级别：</td>
				<td class="inputTd">
				            <c:if test="${org.orgClass eq 3}" >分公司级</c:if>
							<c:if test="${org.orgClass eq 2}" >省级</c:if>
							<c:if test="${org.orgClass eq 1}" >集团级</c:if>	

				</td>
				<td class="inputLabelTd">机构性质：</td>
				<td class="inputTd">
				 <c:if test="${org.orgKind eq 2}" >经营</c:if>
				<c:if test="${org.orgKind eq 1}" >管理</c:if>
				</td>
			</tr>
