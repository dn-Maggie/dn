...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#submit").click(function() {
			...
			var options = {
				url : "<m:url value='/signupUserInfo/update.do'/>",
				type : "post",
				dataType:"json",
				cache : false,
				success : function(d) {
			...
			<table class="table">
				<tr>
					...
					<input
						...
						class="text" value="${userInfo.userId}" />
			    ...
			    <tr>
					...
					<td class="inputTd"><select id="edit_sex" name="sex"
						class="select">
							<option value="1" <c:if test="${userInfo.sex eq 1}" >selected</c:if>>男</option>
							<option value="0" <c:if test="${userInfo.sex eq 0}" >selected</c:if>>女</option>
					</select></td>
			    ...
			    <tr>
					...
					<td colspan="3" class="inputTd">省
						<input id="areaProvinceName" name="areaProvinceName" type="text"
						class="text" value="${userInfo.areaProvinceName}" /> <input
						id="areaProvinceId" name="areaProvinceId" type="hidden"
						class="text" value="${userInfo.areaProvinceId}" /> 市 <input id="areaRegionName"
						name="areaRegionName" type="text" class="text"
						value="${userInfo.areaRegionName}" /> <input id="areaRegionId"
						name="areaRegionId" type="hidden" class="text"
						value="${userInfo.areaRegionId}" /> 县洲区
						<input id="areaCityName" name="areaCityName" type="text"
						class="text" value="${userInfo.areaCityName}" /> <input
