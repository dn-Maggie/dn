	...
	<div id="editDialog">
		<form id="dutyFormEdit">
			<div class="ui-table ui-widget ui-corner-all ui-border">
				<input type="hidden" id="edit_id" name="id" type="text" value="${duty.id}"/>	
				<input id="edit_pid" name="pid" type="hidden" class="text"	value="${pid}" />
				<table class="table">
					<tr>
						<td class="inputLabelTd">级别分类：</td>
						<td class="inputTd"><select name="dutySort"
							id="edit_dutySort" class="select">
								<option value="1" <c:if test="${duty.dutySort eq 1}" >selected</c:if>>集团级</option>
								<option value="2" <c:if test="${duty.dutySort eq 2}" >selected</c:if>>省级</option>
								<option value="3" <c:if test="${duty.dutySort eq 3}" >selected</c:if>>分公司级</option>
						</select></td>
						<td class="inputLabelTd">所属部门：</td>
						<td class="inputTd">
							<select name="orgId" id="edit_orgId" class="select">
								<c:forEach var="orgList" items="${orgList}">
									<option value="${orgList.id}" <c:if test="${orgList.id eq pid}" >selected</c:if>><c:out value="${orgList.orgName}"></c:out></option>
								</c:forEach>
							</select>
						</td>
					...
					<tr>
						...
						<td class="inputTd"><input id="dutyName" name="dutyName"
							type="text" class="text" value="${duty.dutyName}" />
						</td>
