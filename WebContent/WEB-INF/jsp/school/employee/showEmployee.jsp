	...
	<div class="ui-table ui-widget ui-corner-all ui-border">
		<table class="table forview">
			...
			<tr>
				...
				<td class="inputTd">
					<c:if test="${employee.currState eq 1}"><c:out value="在职"></c:out></c:if>
					<c:if test="${employee.currState eq 2}">
						<c:out value="试用"></c:out>
							<c:if test="${employee.probation eq 1}"><c:out value="一个月"></c:out></c:if>
							<c:if test="${employee.probation eq 2}"><c:out value="两个月"></c:out></c:if>
							<c:if test="${employee.probation eq 3}"><c:out value="三个月"></c:out></c:if>
					</c:if>
					<c:if test="${employee.currState eq 3}"><c:out value="离职"></c:out></c:if>
					<c:if test="${employee.currState eq 4}"><c:out value="兼职"></c:out></c:if>
				</td>
