			...
			<table class="table">
				<tr>
					...
					<td class="inputTd">
						...
						<input id="edit_deptName" name="deptName" type="text" class="text"  list="orgList" onchange="getOrgIDByName(this,this.value);" value="${expenseAccount.deptName}"/>
						<datalist id="orgList">
							<c:forEach var="org" items="${org}">
								<option value="${org.orgName}" did="${org.id}" label="${org.orgFullName}" <c:if test="${expenseAccount.deptName==org.orgName}">selected</c:if>></option>
				            </c:forEach>
						</datalist>
					</td>
					...
					<td class="inputTd">
						...
						<input id="edit_enterName" name="enterName" type="text" class="text"  list="empList" onchange="getEmpIDByName(this,this.value);" value="${expenseAccount.enterName}"/>
						<datalist id="empList">
							<c:forEach var="tutor" items="${tutor}">
								<option value="${tutor.nickName}" did="${tutor.id}" label="${tutor.name}" <c:if test="${expenseAccount.enterName==tutor.nickName}">selected</c:if>></option>
				            </c:forEach>
						</datalist>
					</td>
				</tr>
				...
				<tr class="trDetails" style="height:100px;">
					...
					<td class="inputTd" colspan="3">
						<textarea  class="text" name="content" id="edit_content">${expenseAccount.content}</textarea>
					</td>	       
				</tr>
				<tr>
					<td class="inputLabelTd">
