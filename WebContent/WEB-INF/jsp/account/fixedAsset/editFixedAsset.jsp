    ...
    <div class="wrap" >
		...
		<table class="table">
			...
			<tr>
				...
				<td class="inputTd">
					<select class="input_select text" name="assetType" id="edit_assetType" mainid="assetType" >
							<option value="001-房屋、建筑物" <c:if test="${fixedAsset.assetType=='001-房屋、建筑物'}">selected</c:if>>001-房屋、建筑物</option>
							<option value="002-机器机械生产设备" <c:if test="${fixedAsset.assetType=='002-机器机械生产设备'}">selected</c:if>>002-机器机械生产设备</option>
							<option value="003-器具、工具、家具" <c:if test="${fixedAsset.assetType=='003-器具、工具、家具'}">selected</c:if>>003-器具、工具、家具</option>
							<option value="004-运输工具" <c:if test="${fixedAsset.assetType=='004-运输工具'}">selected</c:if>>004-运输工具</option>
							<option value="005-电子设备" <c:if test="${fixedAsset.assetType=='005-电子设备'}">selected</c:if>>005-电子设备</option>
							<option value="006-其他固定资产" <c:if test="${fixedAsset.assetType=='006-其他固定资产'}">selected</c:if>>006-其他固定资产</option>
					</select>
				</td>
			...
			<tr>
				...
				<td class="inputTd">
					<select class="input_select text" name="useOrg" id="edit_useOrg" mainid="useOrg">
						<c:forEach var="org" items="${org}">
							<option value="${org.orgName}" <c:if test="${fixedAsset.useOrg==org.orgName}">selected</c:if>>${org.orgName}</option>
					    </c:forEach>
				    </select>
				</td>
				<td class="inputLabelTd"><span class="required">*</span>使用人：</td>
				<td class="inputTd">
					<input id="edit_applyName" name="applyName" type="hidden" class="text"/>
			...
			<tr>
				...
				<td class="inputTd">
					<select class="input_select text" name="depreMethod" id="edit_depreMethod" mainid="depreMethod">
				...
				<td class="inputTd">
					<label class="radio"><input type="radio" name="isDeprenow" id="edit_isDeprenow1" value="是" <c:if test="${fixedAsset.isDeprenow=='是'}">checked</c:if>>是</label>
					<label class="radio"><input type="radio" name="isDeprenow" id="edit_isDeprenow2" value="否" <c:if test="${fixedAsset.isDeprenow=='否'}">checked</c:if>>否</label>			
				</td>
			</tr>
			<tr>
				...
				<td class="inputTd">
					<input id="edit_zjfySubject" name="zjfySubject" type="text" class="text" value="管理费用-折旧费" onclick="chooseZJSub(this)" onblur="hideZjSubjectList()" value="${fixedAsset.zjfySubject}"/>
					<div class="zjSubjectList">
			           	<ul class="allZjSubjectList" style="position:absolute">
			           		<c:forEach var="zjSubjectList" items="${zjSubjectList}">
			           		 <li aname="${zjSubjectList.accountName}" onclick="fillName(this);">${zjSubjectList.accountId} - ${zjSubjectList.accountName}</li>
			           		</c:forEach>
			           	</ul>
			        </div>
				</td>
			...
			<tr>
				...
				<td class="inputTd">
					<input id="edit_remainRatio" name="remainRatio" type="text" class="text" value="${fixedAsset.remainRatio}" onblur="countYJCZ();"/>&nbsp;%
				</td>
			...
			<tr>
				...
				<td class="inputTd">
					<input id="edit_usePeriod" name="usePeriod" type="text" class="text" value="${fixedAsset.usePeriod}" onblur="countRemainMonth();countPerDepre();"/>
				</td>
				<td class="inputLabelTd"><span class="required">*</span>已折旧月份：</td>
				<td class="inputTd">
					<input id="edit_depredMonth" name="depredMonth" type="text" class="text" value="${fixedAsset.depredMonth}" onblur="countRemainMonth();countPerDepre();"/>
				</td>
			...
			<tr>
				...
				<td class="inputTd">
					<input id="edit_usedDepre" name="usedDepre" type="text" class="text" value="${fixedAsset.usedDepre}" onblur="countPerDepre();"/>
				</td>
