			...
			<div class="search border-bottom">
				...
				<li><input type="text" name="qq" id="qq" class="search_choose"> <span>QQ号码:</span></li><!-- 输入框-->
 				<li class="date_area">
					...
					<li>
						<select class="search_choose" name="courseid" id="courseid" mainid="courseid">
...
var gridObj = {};
	$(function(){
  		gridObj = new biz.grid({
            ...
            url: "<m:url value='/marketStudent/listKouBeiStudent.do'/>",/*grid初始化请求数据的远程地址*/
            ...
            colModel:[
				...
				{name : "tel",label:"电话",index : "tel"},
				{name : "coursename",label:"意向课程",index : "coursename"},							
				{name : "entry_emp",label:"录入人",index : "entry_emp"},				
				{name : "time",label:"录入时间",index : "time"},
	...
	var show_iframe_dialog;
  	...
  	function add(){
  		var url="<m:url value='/marketStudent/toAddKouBeiStudent.do'/>";
    ...
    function edit(){
		...
		var url="<m:url value='/marketStudent/toEditKouBeiStudent.do'/>?key="+key;
    ...
    function show(){
		...
		var url="<m:url value='/marketStudent/toShowKouBeiStudent.do'/>?key="+key;
  ...
  //报名
    function baomin(){
		...
		var url="<m:url value='/marketStudent/kouBeiPagtoBM.do'/>?key="+key;
		show_iframe_dialog = new biz.dialog({
		 	id:$('<div id="showwindow_iframe"></div>').html('<iframe id="iframeShow" name="iframeShow" src="'+url+'" width="100%" frameborder="no" border="0" height="97%"></iframe>'),  
			modal: true,
			width: $(window).width()*0.6,
    ...
    function batchDelete(){
    	...
    	if(ids==""){
    		...
    		new biz.alert({type:"confirm",message:I18N.msg_del_confirm,title:I18N.promp,callback:function(result){
    			if(result){
    				$ .ajax({
        				url: "<m:url value='/marketStudent/deleteKouBeiStudent.do'/>?key="+ids,
        				cache:false,
