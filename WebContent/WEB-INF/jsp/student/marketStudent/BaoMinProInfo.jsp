		...
		<table class="table">
			...
			<tr>
				...
				<td class="inputTd">
					...
					<input class="text" name="courseName" id="edit_courseName" list="courseList" onchange="getIdByName(this,this.value)" />	
						<datalist id="courseList">
							<c:forEach var="cour" items="${er.kechen}">
								<option value="${cour.name}" did="${cour.id}" label="${cour.name}" ></option>
				            </c:forEach>
						</datalist>
				</td>
			</tr>
			<tr>
				...
				<td class="inputTd">
					<div class="time_bg">
						<input type="text" class="search_time150 valid" name="joinTime" id="edit_joinTime" mainid="joinTime"><!-- 时间选择控件-->
						<i class="search_time_ico2"></i>
					</div>
				</td> 
				<td class="inputLabelTd">备注：</td>
				<td class="inputTd">
					<input id="edit_notes" name="notes" type="text" class="text" />
				</td>
			</tr>
			<tr>
...
function addVipStudent(){
	...
	$.ajax({
		   type: "post",
		   url: "<m:url value='/marketStudent/proInfobaomin.do'/>?piid=${marketStudent.id}",
		   data: paramDatas,
		   cache: false,
		   dataType:"json",
		   success:function(d){
