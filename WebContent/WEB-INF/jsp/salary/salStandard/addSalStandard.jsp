...
<body>
			...
			<div class="ui-table ui-widget ui-corner-all ui-border">
				...
				<table class="table">
					<tr>
						...
						<td class="inputTd">
							<select id="levelCode" name="levelCode" class="select" onChange="changeStandardName()">
								<option value=""></option>
								<c:forEach var="dutyLevel" items="${dutyLevelList}">
									<option value="${dutyLevel.dictCode }">${dutyLevel.dictName }</option>
								</c:forEach>
							</select>
							<input type="hidden" id="leveName" name="leveName" type="text"
							value="" /></td>
					</tr>
					<tr>
					...
					<tr>
						<td class="inputLabelTd">
							<span class="required">*</span>标准名称：
						</td>
						<td class="inputTd">
							<input id="standardName" name="standardName" type="text" class="text" value="${salStandard.standardName}" />
						</td>
						<td class="inputLabelTd">
							<span class="required">*</span>薪资总额：
						</td>
						<td class="inputTd">
							<input id="edit_totalValue" name="totalValue" type="text" class="text" value="${salStandard.totalValue}" />
						</td>
					</tr>
					...
					<tr>
						...
						</select> <input type="hidden" id="salaryTypename" name="salaryTypename"
							type="text" value="" /></td>
					...
					<tr>
						...
						<td class="inputTd" colspan="3"><input id="edit_remark"
							name="remark" type="text" class="text" style="width: 96.6%;"
							value="${salStandard.remark}" /></td>
					</tr>
	...
	$(function() {
		...
		$("#submit").click(function() {
							$('#leveName').val($('#levelCode').find("option:selected").text());
							$('#dutyName').val($('#dutyId').find("option:selected").text());
							$('#salaryTypename').val($('#salaryTypeCode').find("option:selected").text());
							if (!biz.validate("valid",
									$('#salStandardFormEdit')[0])) {
								showWarn("数据验证失败", 3000);
								return;
							}
							var options = {
								url : "<m:url value='/salStandard/addSalStandard.do'/>",
								type : "post",
								dataType : "json",
								success : function(d) {
									if (d.status) {
